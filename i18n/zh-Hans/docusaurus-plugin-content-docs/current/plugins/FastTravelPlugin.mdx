---
description: 允许玩家传送到指定点位或赛道上的任意位置。
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import CodeBlock from '@theme/CodeBlock';
import cfg from "!!raw-loader!./reference/plugin_fast_travel_cfg.reference.yml";

# FastTravelPlugin

:::note

该插件强制要求最低 CSP 版本 0.2.8 (3424)，并且需要在 `extra_cfg.yml` 中设置 `EnableClientMessages: true` 和 `EnableAi: true`！  

:::

## 功能
* 允许玩家传送到赛道上的任意位置。
* 支持 CSP 传送点（在 `csp_extra_options.ini` 中配置）
* 传送后短时间内禁用碰撞，以降低被恶意利用的风险。
...
<TabItem value="imola" label="Imola 示例">
...
## 传送目标配置  
你还可以让 `csp_extra_option.ini` 中配置的传送目标变为可选点。  
**如果你还没有这个文件并且想要添加点位，请先阅读[这个 FAQ 章节](../faq.md#csp-extra-options)。**

默认情况下，点位共有 3 种"类型"：
  - `SP`：默认类型，仅在放大到较近的缩放级别时可见  
     ![](../assets/smeeZH9.png)
  - `PA`：可分配给点位（或分组，稍后详述），即使完全缩小地图也可见。  
    ![](../assets/sLQl81Y.png)   
  - `ST`：与 `PA` 相同，只是图标不同  
    ![](../assets/rBCC9xs.png)  
  ...
  将更多 `mapicon_<TYPE>.png` 图片放入 `\plugins\FastTravelPlugin\wwwroot\` 文件夹，即可添加新的类型和图标。  
  例如，添加 `mapicon_gs.png` 后，重启服务器即可使用 `POINT_<NUM>_TYPE = GS`。   

  :::

所有点位默认都会被视为 `SP`，你可以为传送点添加 `POINT_<NUM>_TYPE = <TYPE>` 来更改其类型。  
...
<details>
<summary>**基于 Shutoko Revival Project 官方传送点位置的传送配置**</summary>
...
请确保你已在 CSP 聊天应用设置中启用管理员选项。  
打开聊天应用 > 点击齿轮图标 > 在 `Administrating` 下 > 勾选 `Enable admin options`  
然后点击锤子图标登录。  

:::

以管理员身份登录后，你可以使用 FastTravelDebug 扩展选项来调整地图图片的位置、缩放和移动速度级别。  
只需点击聊天应用中的灯泡图标并选择 `FastTravelDebug...` 即可。  
请注意，在你将这些更改应用到 `plugin_fast_travel_cfg.yml` 并重启服务器之前，其他玩家是看不到这些更改的。
